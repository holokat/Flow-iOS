...
struct FlowApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            if KeyboardPredictionDiagnosticsView.isEnabled {
                KeyboardPredictionDiagnosticsView()
            } else {
                appContent
            }
        }
...
#if DEBUG
private struct KeyboardPredictionDiagnosticsView: View {
    static var isEnabled: Bool {
        ProcessInfo.processInfo.arguments.contains("--flow-keyboard-diagnostics")
    }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                Text("Type slowly in each field. These controls intentionally keep default text-input traits.")
                    .font(.body)
                    .foregroundStyle(.secondary)

                VStack(alignment: .leading, spacing: 8) {
                    Text("UIKit-only controller")
                        .font(.headline)
                    UIKitKeyboardDiagnosticsHost()
                        .frame(height: 210)
                        .background(.regularMaterial, in: RoundedRectangle(cornerRadius: 8))
                }

                VStack(alignment: .leading, spacing: 8) {
                    Text("SwiftUI-wrapped UITextField")
                        .font(.headline)
                    BareKeyboardDiagnosticsTextField(text: $textFieldText)
                        .frame(height: 44)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Text("SwiftUI-wrapped UITextView")
                        .font(.headline)
                    BareKeyboardDiagnosticsTextView(text: $textViewText)
                        .frame(minHeight: 140)
                        .padding(8)
                        .background(.regularMaterial, in: RoundedRectangle(cornerRadius: 8))
                }
...
private struct UIKitKeyboardDiagnosticsHost: UIViewControllerRepresentable {
    func makeUIViewController(context: Context) -> UIKitKeyboardDiagnosticsViewController {
        UIKitKeyboardDiagnosticsViewController()
    }

    func updateUIViewController(_ uiViewController: UIKitKeyboardDiagnosticsViewController, context: Context) {}
}

private final class UIKitKeyboardDiagnosticsViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        textField.addTarget(self, action: #selector(updateStatus), for: .editingChanged)
        ...
        let stack = UIStackView(arrangedSubviews: [statusLabel, textField, textView])
        stack.axis = .vertical
        stack.spacing = 10
    ...
    @objc private func updateStatus() {
        let activeInput = [textField, textView].first(where: \.isFirstResponder)
        let language = activeInput?.textInputMode?.primaryLanguage ?? "none"
        statusLabel.text = "Active input language: \(language)"
    }
}

private struct BareKeyboardDiagnosticsTextField: UIViewRepresentable {
...
private struct BareKeyboardDiagnosticsTextView: UIViewRepresentable {
    @Binding var text: String
